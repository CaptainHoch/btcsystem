...
def flush_geometry():
    ...
    #
    poly_q = ('triangles', poly_draw_queue, 'poly')
    bg_q = ('triangles', background_draw_queue, 'bg')
    line_q = ('lines', line_draw_queue, 'line')
    point_q = ('points', point_draw_queue, 'point')

    for draw_type, draw_queue, name in [bg_q, line_q, poly_q, point_q]:
        ...
        sidx = 0
        draw_indices = []
        for vertices, idx, color, stroke_weight in draw_queue:
...
def add_to_draw_queue(stype, vertices, edges, faces, fill=None, stroke=None, stroke_weight=None):
    ...
    if stype == 'background':
        ...
        background_draw_queue.append((vertices, idx, fill, None))
        return

    if fill_shape and stype not in ['point', 'path']:
        idx = np.array(faces, dtype=np.uint32).ravel()
        poly_draw_queue.append((vertices, idx, fill, stroke_weight))

    if stroke_shape:
        if stype == 'point':
            idx = np.arange(0, len(vertices), dtype=np.uint32)
            point_draw_queue.append((vertices, idx, stroke, stroke_weight))
        else:
            idx = np.array(edges, dtype=np.uint32).ravel()
            line_draw_queue.append((vertices, idx, stroke, stroke_weight))
